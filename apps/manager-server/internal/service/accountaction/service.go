...
func (s *Service) DeleteAuthFile(ctx context.Context, id int64) (model.AccountActionCandidate, error) {
	...
	// CPA has committed the deletion. Cleanup runs after candidate persistence
	// so the status transition can still be recorded, and also runs when that
	// write needs a retry or ultimately fails.
	cleanupErr := s.store.CleanupDeletedCredential(ctx, model.CredentialIdentity{
	...
	if persistErr == nil {
		if cleanupErr != nil {
			s.recordCandidateFailure(ctx, id, cleanupErr)
		}
		return updated, nil
	}
	resultErr := fmt.Errorf(
