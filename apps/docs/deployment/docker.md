...
services:
  cli-proxy-api:
    ...
    volumes:
      # config.yaml must be generated by the installer or copied from the
      # pinned release template before `docker compose up`.
...
```bash
mkdir -p cliproxyapi/{auths,logs,data/license} secrets
# The installer writes cliproxyapi/config.yaml with the management key and
# license section. For a manual install, create that file from the pinned CPA
# release template and set remote-management.secret-key before continuing.
